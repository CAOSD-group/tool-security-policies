...
def validate_all_configs(flat_fm, z3_model, processed_files):
    ...
    with open(OUTPUT_CSV, mode='a', newline='') as f:
        ...
        # Sorted the files reverse ## Divide from the end to have faster feedback on recent files (assuming they are added at the end)
        all_files_reversed = sorted(
            [f for f in os.listdir(VALID_JSONS_DIR) if f.endswith(".json")], 
            reverse=True
        )
        total_archivos = len(all_files_reversed)
        mitad = total_archivos // 2

        # PARA EL PC 2 (Procesará el primer bloque de la lista invertida, ej. de la Z a la M)
        # my_files = all_files_reversed[ : mitad ]
        
        # PARA EL PC 3 (Procesará el segundo bloque de la lista invertida, ej. de la M a la A)
        #my_files = all_files_reversed[ mitad : ]

        #print(f"Se van a procesar {len(my_files)} de {total_archivos} archivos en esta máquina.")
        # ==========================================================
